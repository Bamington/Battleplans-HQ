...
export default function StarcraftCardForm({
  ...
  const [f, setF] = useState<Fields>({
    unitType: '', unitName: '', tags: '',
    speed: 0, evade: 0, armour: 0, hitPoints: 0, size: 0, pointsCost: 0,
    supplyTiers: [{ supply: 0, maxModels: 1 }],
  });
  ...
  const handleRuleSave = useCallback(async (name: string, desc: string, stats: unknown): Promise<string> => {
    ...
    setSavingR(true);
    const { data, error: err } = await supabase.from('addons')
      .insert({ addon_type_id: ruleType.id, pack_id: packId, name, description: desc, stats: stats as Json })
      .select('id').single();
  ...
  return (
    <div className="flex flex-col gap-6 p-5">
      <h2 className="font-heading text-xl text-white">
        {f.unitType || 'New Unit'} — Add Content
      </h2>
