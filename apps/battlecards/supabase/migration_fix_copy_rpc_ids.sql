-- Fix copy_keywords_to_pack and copy_addons_to_pack to return the
-- target-pack IDs (new or reused) instead of an integer count.
-- Previously both functions had `return v_count` despite being declared
-- `returns uuid[]`, causing the modal to receive no IDs and therefore
-- never attach copied addons/keywords to the card via card_addons /
-- card_keywords.
-- PostgreSQL won't allow changing return type via CREATE OR REPLACE,
-- so we drop first.

drop function if exists public.copy_keywords_to_pack(uuid, uuid[]);
drop function if exists public.copy_addons_to_pack(uuid, uuid[]);
...
-- copy_addons_to_pack already builds v_addon_map (source → target),
-- so we just return the target IDs from that map.
...
begin
  ...
  -- addon_keywords joins only for newly-inserted addons.
