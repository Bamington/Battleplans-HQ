...
// ── Thumbnail ─────────────────────────────────────────────────────────────────
// A square that bleeds to the card edges. Prefers the model's own photo,
// falls back to the game icon, then to the model name on a dark tile.

export function CollectionThumb({ imageUrl, iconUrl, name, className = 'w-[108px] h-[108px]' }: {
...
/** A 260px hero image for a gallery card: the photo, or a blurred game-icon
 *  backdrop with the crisp icon centred, or the name on a flat tile. */
function CardHero({ imageUrl, iconUrl, name }: { imageUrl: string | null; iconUrl: string | null; name: string }) {
  return (
...
export function ModelGridItem({ model, onClick }: { model: CollectionModel; onClick?: () => void }) {
  ...
  return (
    ...
    >
      <CardHero imageUrl={modelImageUrl(model.imagePath)} iconUrl={iconUrl} name={model.name} />
